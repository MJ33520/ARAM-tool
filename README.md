...
编辑 `config.py`，修改 `DISPLAY_MODE`：

```python
# "moba"   = 显示器开启了 MOBA/游戏 模式（金色名字可能偏暗）
# "normal" = 普通显示模式（金色名字正常显示为亮金色）
DISPLAY_MODE = "moba"   # ← 改成 "normal" 如果你没开游戏模式
```

**如何判断用哪个模式？**
- 如果你的显示器开启了「游戏模式」「MOBA 模式」等画面预设 → 用 `"moba"`
- 如果是默认/标准显示模式 → 用 `"normal"`
- 如果 AI 经常识别错你的英雄，试试切换模式
